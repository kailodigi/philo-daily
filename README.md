...
- 每日按统一金融、AI、半导体、社媒采集联网候选。统一金融池最多执行 3 轮条件式搜索：每轮完成来源绑定、时效过滤和去重后，达到至少 7 条候选且满足“至少 2 条市场/宏观/政策、至少 2 条公司/行业”即停止，不再调用后续 query；最终按重要性选择其余 1 条不限的 Top 5。最后只用 1 次不联网调用生成 V3 正文。
...
HTML 生成前，独立质量门禁会检查金融和 AI 各至少 3 条、最多 5 条；5 条是目标而非凑数要求。每条资讯仍必须包含状态/事实/重要性/来源/发布时间，来源链接属于已验证搜索候选，超过 48 小时的资讯必须标为“延续”。金融或 AI 少于 5 条时，页面会明确说明没有使用旧闻或低可信来源补足数量。社媒允许 0–5 条；可靠公开信号不足 3 条时不凑数，并显示“今日可靠社媒趋势不足”。随后再检查当天 HTML 非空、可按 UTF-8 读取、结构完整、无明显占位符，以及全部内部链接有效。
...
工作流先运行不联网单元测试，通过后才允许调用 API。金融搜索根据候选数量与结构在第 1、2 或 3 轮停止，AI、半导体和社媒保持各自现有采集规则；API 请求失败最多重试一次，正文结构不合格也只允许重试一次，并限制候选与正文输出长度。`data/usage/YYYY-MM.json` 按 workflow run 记录日期、真实 run id、模型、成功/失败状态、输入/输出 Token 及人民币和美元费用估算；生成或后续质量检查失败时也只提交失败用量快照，随后以非零状态退出，不会部署或覆盖正常首页。月度估算达到 4.75 美元时会在调用前停止，以保留 5 美元预算余量。估算采用仓库记录的保守单价与固定汇率，阿里云实际账单和汇率可能不同，应以控制台为准。
