...
class SelectedCandidateItem(StrictModel):
    ...
    what_happened: str = Field(min_length=20, max_length=110)
    why_important: str = Field(min_length=20, max_length=110)
    source_id: str = Field(pattern=r"^[FAS]\d+$")
    published_at: str = Field(min_length=10, max_length=48)
    published_date: str = Field(pattern=r"^\d{4}-\d{2}-\d{2}$")
    confidence: Literal["高", "中", "低"]
    continuation_of: str | None
...
class SelectedCandidateBatch(StrictModel):
    candidates: list[SelectedCandidateItem] = Field(min_length=15, max_length=15)
...
def collect_candidates(
    *,
    ...
    for plan_number, plan in enumerate(SEARCH_PLANS):
...
"""
        ...
        search_results = search_result_records(response)[: plan["limit"] + 4]
        source_urls = {normalize_url(result["url"]) for result in search_results}
        ...
        for category, minimum in plan["minimum"].items():
            ...
            if eligible_count < minimum:
                raise ValueError(
                    f"{plan['name']} has only {eligible_count} timely {category} candidates; need {minimum}"
                )
        prefix = ("F", "A", "S")[plan_number]
...
硬性要求：
- 恰好选择 15 条：全球金融 5、AI行业 5、半导体重点 3、社媒趋势 2。
- source_id 必须逐字来自真实来源目录，且 15 条不得重复 source_id；不要输出 URL 或来源名。
...
"""
    ...
    selected_ids = [item.source_id for item in selected.candidates]
    if len(selected_ids) != len(set(selected_ids)):
        raise ValueError("Qwen reused a source_id while binding candidate sources")

    all_candidates: list[CandidateItem] = []
    for item in selected.candidates:
        source = source_catalog.get(item.source_id)
        if source is None:
            raise ValueError(f"Unknown source_id returned by Qwen: {item.source_id}")
        if item.category not in source["category_scope"]:
            raise ValueError(
                f"Source {item.source_id} is outside the {item.category} search scope"
            )
        source_number = int(re.search(r"\d+", item.source_id).group())
        all_candidates.append(
            CandidateItem(
                **item.model_dump(exclude={"source_id"}),
