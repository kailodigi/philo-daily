...
class SelectedCandidateBatch(StrictModel):
    selections: dict[str, SelectedCandidateItem] = Field(min_length=15, max_length=15)
...
- selections 必须是以真实 source_id 为键的 JSON 对象，恰好包含 15 个不同键；每个键必须逐字来自真实来源目录；不要输出 URL 或来源名。
