...
class SelectedCandidateBatch(StrictModel):
    selections: dict[str, SelectedCandidateItem] = Field(min_length=17, max_length=17)
...
硬性要求：
- 恰好选择 17 条：全球金融 5、AI行业 5、半导体重点 4、社媒趋势 3。半导体与社媒各多选 1 条作为事件去重的备用候选。
- selections 必须是以真实 source_id 为键的 JSON 对象，恰好包含 17 个不同键；每个键必须逐字来自真实来源目录；不要输出 URL 或来源名。
- 只在候选事实与来源标题语义对应时绑定；无法可靠对应的候选不得采用。
- 同一事件即使来源不同也只能选择一次；半导体 4 条和社媒 3 条必须分别描述不同事件。
